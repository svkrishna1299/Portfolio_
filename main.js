...
const PROJECTS = {
  ...
  'foiling-craft': {
    ...
    description: `Hydrofoil craft lift a vessel clear of the water to drastically cut drag and energy consumption — but in rough seas, the oscillating inflow makes maintaining a stable lift force genuinely hard. This project built and validated a <strong>real-time PD controller</strong> embedded directly inside a STAR-CCM+ CFD simulation via Java macros, actively rotating a NACA 0012 hydrofoil to track a constant 5,000 N lift target as a sinusoidal wave velocity field sweeps past it.<br><br>The controller logic runs <em>inside</em> the solver at every time step: reading the instantaneous lift force, computing the error against the setpoint, and commanding a new angle of attack — all while the flow physics continue to evolve around the moving foil.`,
    methodology: `A 2D computational domain of 6×6 m was built around a NACA 0012 hydrofoil (Re ≈ 2.2×10⁶) with an overset (chimera) mesh allowing free rotation of the foil without remeshing. The background mesh covered the full domain with wake refinement to the outlet; the overset region surrounded the foil with 15 prism layers targeting y+ ≤ 1 (near-wall thickness 2.5×10⁻⁶ m, 89,036 total cells). Implicit Unsteady RANS with SST k–ω and All y+ wall treatment was used throughout. The inlet velocity followed V(t) = 10 + 2sin(2nπt) m/s, with wave frequencies of 0.5, 1, and 2 Hz tested. The PD controller was first prototyped and gain-swept analytically in Python, reducing the feasible gain space before embedding the best candidates into the STAR-CCM+ Java macro. Simulation validation: CFD C_L matched Houghton et al. experimental data across AoA 0–10° to within 13%. Two gain sets were identified — a stable set (K_p=2.5×10⁻⁵, K_d=1.0×10⁻⁷) maintaining controlled oscillation, and an unstable set demonstrating divergence — confirming the sensitivity of embedded real-time control to gain selection.`,
    results: [
    ...
    images: [
      { file: 'cover.jpg',  caption: 'Velocity contour field — NACA 0012 hydrofoil at angle of attack under unsteady inflow' },
      { file: 'cfd1.png',   caption: 'Overset mesh topology — background domain with rotating hydrofoil sub-region and prism layers' },
      { file: 'cfd2.png',   caption: 'CFD validation — simulated vs. experimental C_L across angle of attack 0–10°' },
      { file: 'plot1.png',  caption: 'PD controller stability — stable vs. unstable gain sets under constant velocity (C_V = 10 m/s)' },
      { file: 'plot2.png',  caption: 'Core result: lift force tracking and AoA actuation under 1 Hz sinusoidal inlet velocity' },
      { file: 'plot3.png',  caption: 'Wave frequency sweep — lift response across 0.5 / 1 / 2 Hz for three gain combinations' },
    ],
  },
